...
qy = lognormal_rate_distribution(mu*np.ones(nmax+1),nu*np.ones(nmax+1),
                                  rate,ymax,nmax)
...
for j,rho in enumerate(rho_vec):
    if rho in results['special_rho_list']:
        ...
        zorder=0
        lw=0.7
    ax.semilogy(ivec[1:],results['eff_rate_vec'][j],
         color=newcm(j/len(rho_vec)),
         alpha=alpha,label=label,zorder=zorder,lw=lw)
ax.semilogy(ivec[1:],0.016*np.exp(0.2*ivec[1:]),'--',color='k',
                                         label=r"$\sim e^{i/5}$")
...
with open('./dat/stationary_effective_rate_lognormal.pk', 'wb') as outfile:
