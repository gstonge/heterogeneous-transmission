...
mu = 0.01
nu = 1
rate = np.linspace(0.001,0.5,ymax+1)

qy = weibull_rate_distribution(mu*np.ones(nmax+1),nu*np.ones(nmax+1),
                                  rate,ymax,nmax)
...
for j,rho in enumerate(rho_vec):
    if rho in [10**(-5),0.1,1,10**3]: #or rho==1:
        alpha = 1
        ...
        zorder=0
        lw=0.7
    ax.loglog(ivec[1:],results['eff_rate_vec'][j],
         color=newcm(j/len(rho_vec)),
         alpha=alpha,label=label,zorder=zorder,lw=lw)
ax.loglog(ivec[1:],0.02*ivec[1:],'--',color='k',label=r"$\sim i$")
...
with open('./dat/stationary_effective_rate_weibull.pk', 'wb') as outfile:
